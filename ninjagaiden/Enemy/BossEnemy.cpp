...
BossEnemy::BossEnemy(Vector2 leftbot_wP, Vector2 size)
{
	position.x = leftbot_wP.x + size.x - 10;
...
BndBox BossEnemy::getBndBox(float dtTime)
{
	if (state == EnemyState::DEAD) return BndBox();

	BndBox bb;
	bb.size = anicollectS[ObjKind::Boss]->getAnimation(state)->getCurFrameSize();
	Vector2 center = Vector2(bb.size.x / 2, bb.size.y);
